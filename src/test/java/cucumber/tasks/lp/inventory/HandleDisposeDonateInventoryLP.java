...
public class HandleDisposeDonateInventoryLP {

    public static Task search(Map<String, String> info) {
        return Task.where("",
                Check.whether(!info.get("number").isEmpty()).andIfSo(
                        Enter.theValue(info.get("number")).into(WithdrawalRequestLPPage.D_FIELD_SEARCH("Number")).thenHit(Keys.ENTER)
                ),
                Check.whether(!info.get("vendorCompany").isEmpty()).andIfSo(
                        Enter.theValue(info.get("vendorCompany")).into(WithdrawalRequestLPPage.D_FIELD_SEARCH("Vendor Company")).thenHit(Keys.ENTER)
                ),
                ...
                Check.whether(!info.get("type").isEmpty()).andIfSo(
                        CommonTask.chooseItemInDropdown(WithdrawalRequestLPPage.D_FIELD_SEARCH("Type"), CommonLPPage.DYNAMIC_ITEM_DROPDOWN_1(info.get("type")))
                ),
                Check.whether(!info.get("region").isEmpty()).andIfSo(
                        CommonTask.chooseItemInDropdown(WithdrawalRequestLPPage.D_FIELD_SEARCH("Region"), CommonLPPage.DYNAMIC_ITEM_DROPDOWN_1(info.get("region")))
                ),
    ...
    public static Task inputSearchAll(Map<String, String> map) {
        return Task.where("search all order",
                ...
                Check.whether(map.get("number").isEmpty()).otherwise(
                        ...
                        CommonWaitUntil.isNotVisible(CommonLPPage.LOADING_ICON("Fetching your"))
                ),
                ...
                Check.whether(map.get("type").isEmpty()).otherwise(
                        CommonTask.chooseItemInDropdown1(CommonLPPage.DYNAMIC_SEARCH_FIELD("Type"), CommonLPPage.DYNAMIC_ITEM_DROPDOWN_2(map.get("type"))),
                        CommonWaitUntil.isNotVisible(CommonLPPage.LOADING_ICON("Fetching your"))
                ),
