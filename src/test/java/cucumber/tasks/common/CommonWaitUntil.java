...
public class CommonWaitUntil {
    ...
    public static Interaction waitToDownloadSuccessfully(String fileName) {
        return (Interaction) Wait.until(CommonQuestions.checkFileExist(fileName, System.getProperty("user.dir") + "/target/"), equalTo(true)).forNoMoreThan(GVs.HTTP_TIMEOUT).seconds();
    }

    public static Interaction waitToDownloadSuccessfully(String path, String fileName) {
        return (Interaction) Wait.until(CommonQuestions.checkFileExist(fileName, path), equalTo(true)).forNoMoreThan(GVs.HTTP_TIMEOUT).seconds();
    }

    public static Interaction waitToLoadingNewWindow(String pageTitle) {
        return Wait.until(CommonQuestions.pageTitle(), equalTo(pageTitle)).forNoMoreThan(GVs.HTTP_TIMEOUT).seconds();
    }
    public static Interaction waitToLoadingNewWindow3(String pageTitle) {
        return Wait.until(CommonQuestions.pageTitle(), containsString(pageTitle)).forNoMoreThan(GVs.HTTP_TIMEOUT).seconds();
    }
