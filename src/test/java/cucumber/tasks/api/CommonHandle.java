...
//        Serenity.recordReportData().asEvidence().withTitle("EMAIL").andContents(email);
    ...
    public static String setDate2(String dateStr, String formatDate) {
        String date = "";
        String day = "0";
        if (dateStr == null) {
            dateStr = "";
        ...
        switch (dateStr) {
            case "currentDate":
                date = Utility.getDateTimeFromToday(formatDate, 0);
                break;
            case "tomorrow":
                date = Utility.getDateTimeFromToday(formatDate, +1);
                break;
            case "currentMonth+1":
                date = Utility.getMonthFromToday(formatDate, +1);
                break;
            case "currentMonth-1":
                date = Utility.getMonthFromToday(formatDate, -1);
                break;
            case "Plus":
                date = Utility.getDateTimeFromToday(formatDate, Integer.parseInt(day));
                System.out.println("Ngày được tăng thêm " + day + " ngày : " + Utility.getDateTimeFromToday(formatDate, Integer.parseInt(day)));
                break;
            case "Minus":
                date = Utility.getDateTimeFromToday(formatDate, Integer.parseInt(day), "Minus");
                System.out.println("Ngày được trừ đi " + day + " ngày : " + Utility.getDateTimeFromToday(formatDate, Integer.parseInt(day), "Minus"));
                break;
            case "-":
                date = "-";
                break;
            case "currentDateFull":
                LocalDate localDate = LocalDate.now();
                date = DateTimeFormatter.ofLocalizedDate(FormatStyle.FULL).format(localDate);
    ...
    public static String setDateFull(String dateStr) {
        String date = "";
        String day = "0";
        if (dateStr.contains("Plus")) {
            day = dateStr.substring(4);
        ...
        switch (dateStr) {
            ...
            case "currentMonth+1":
                ...
                break;
            case "Plus":
                localDate = localDate.plusDays(Long.parseLong(day));
                break;
            case "Minus":
                localDate = localDate.minusDays(Long.parseLong(day));
                break;
            case "Month+":
                localDate = localDate.plusMonths(Long.parseLong(day));
                break;
            case "Month-":
                localDate = localDate.minusMonths(Long.parseLong(day));
                break;
            default:
                date = dateStr;
                break;
    ...
    /**
     * @param table
     * @param index: from index of keys
     * @return
     */
